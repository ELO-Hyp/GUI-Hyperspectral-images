...
class DRWindow_pca:
    ...
    def __process(self, output_dir: str):
        try:
            ...
            no_bands = 10
            dr_pca = pca.PCA(n_bands = no_bands) # construct object
            dr_pca.fit(hyp_img)
            hyp_img_red = dr_pca.transform(hyp_img, no_bands)
            print(hyp_img_red.shape)

            #hyp_img_recover = dr_pca.inverse_transform(hyp_img_red)
            
            ############### save output  ########################
            path_to_save_projection = os.path.join(output_dir, "projection_pca")
            path_to_save_imgred = os.path.join(output_dir, "reduced_hypImg_pca.mat")
            
            savemat(path_to_save_imgred, {'hyp_img_red' : hyp_img_red})
            dr_pca.save(path_to_save_projection)
