...
################ load weights  ###################
            ssvm2 = ssvm.SSVM()
            ssvm2.load('post_process_data/Trondheim2023.json')
                    
############## load data #####################
#             Tron = np.fromfile('data/trondheim_2023-03-27_0939Z.bip', dtype=np.uint16)
...
#                 Tron2[:,i] = np.array(Tron[i,:,:]).flatten()
            ...
            Tron = scipy.io.loadmat(input_dir)['matrix']
            X = preprocessing.scale(Tron, axis=0)                    # Normalization
            y[y == 0] = 2
            I = [2, 4, 5, 6, 9]
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_Trondheim2023.txt")
            ...
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Trondheim2023.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationTrond2023_red_pca:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ################ load weights  ###################
        ssvm2 = ssvm.SSVM()
        ssvm2.load('post_process_data/Trondheim2023_red_pca.json')
        try:
                    
############## load data #####################
            y = np.load('data/trondheim_2023-03-27_0939Z-labels.npy')
            Tron = scipy.io.loadmat(input_dir)['hyp_img_red']
            Tron2 = Tron.T
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_Trondheim2023_pca.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Trondheim2023_pca.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationTrond2023_red_ica:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ################ load weights  ###################
        ssvm2 = ssvm.SSVM()
        ssvm2.load('post_process_data/Trondheim2023_red_ica.json')
        try:
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_Trondheim2023_ica.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Trondheim2023_ica.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationTrond2023_red_nmf:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ################ load weights  ###################
        ssvm2 = ssvm.SSVM()
        ssvm2.load('post_process_data/Trondheim2023_red_nmf.json')
        try:
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_Trondheim2023_nmf.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Trondheim2023_nmf.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationTrond2023_red_osp:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ################ load weights  ###################
        ssvm2 = ssvm.SSVM()
        ssvm2.load('post_process_data/Trondheim2023_red_osp.json')
        try:
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_Trondheim2023_osp.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Trondheim2023_osp.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationTrond2023_red_lpp:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ################ load weights  ###################
        ssvm2 = ssvm.SSVM()
        ssvm2.load('post_process_data/Trondheim2023_red_lpp.json')
        try:
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_Trondheim2023_lpp.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Trondheim2023_lpp.pdf', bbox_extra_artists=(lgd,), bbox_inches='tight')
