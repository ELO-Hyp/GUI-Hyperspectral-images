...
################ load weights  ###################
            ssvm2 = ssvm.SSVM()
            ssvm2.load('post_process_data/Samson.json')
                    
############## load data #####################
            X = sio.loadmat(input_dir)['V']
            y = sio.loadmat('data/label_samson.mat')['labels']
            X = preprocessing.scale(X.T, axis=0)                    # Normalization
            y = np.reshape(np.float64(y.T), (-1,))
            I = [0, 1, 2]
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_samson.txt")
            ...
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Samson.pdf',  bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationSamson_red_pca:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ...
        try:
            ################ load weights  ###################
            ssvm2 = ssvm.SSVM()
            ssvm2.load('post_process_data/Samson_red_pca.json')
                    
############## load data #####################
#             X = sio.loadmat('data/samson.mat')['V']
            X = scipy.io.loadmat(input_dir)['hyp_img_red']
            y = sio.loadmat('data/label_samson.mat')['labels']
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_samson_pca.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Samson_pca.pdf',  bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationSamson_red_ica:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ...
        try:
            ################ load weights  ###################
            ssvm2 = ssvm.SSVM()
            ssvm2.load('post_process_data/Samson_red_ica.json')
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_samson_ica.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Samson_ica.pdf',  bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationSamson_red_nmf:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ...
        try:
            ################ load weights  ###################
            ssvm2 = ssvm.SSVM()
            ssvm2.load('post_process_data/Samson_red_nmf.json')
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_samson_nmf.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Samson_nmf.pdf',  bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationSamson_red_osp:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ...
        try:
            ################ load weights  ###################
            ssvm2 = ssvm.SSVM()
            ssvm2.load('post_process_data/Samson_red_osp.json')
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_samson_osp.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Samson_osp.pdf',  bbox_extra_artists=(lgd,), bbox_inches='tight')
...
class ClassificationSamson_red_lpp:
    def __init__(self, window, window_title):
    ...
    def __process(self, input_dir: str, output_dir: str):
        ...
        try:
            ################ load weights  ###################
            ssvm2 = ssvm.SSVM()
            ssvm2.load('post_process_data/Samson_red_lpp.json')
...
####################################### to save the output  ###########################
            ### .txt file
            path_to_save = os.path.join(output_dir, "output_details_samson_lpp.txt")
            file1 = open(path_to_save, "w")
            file1.write("Overall Accuracy----\n")
            toFile = "Total number of data for testing ="
            toFile1 = "Correctly predicted = "
            ...
            plt.xlabel("Number of bands")
            plt.ylabel("Weights")
            plt.savefig(output_dir+'/SVM_weights_Samson_lpp.pdf',  bbox_extra_artists=(lgd,), bbox_inches='tight')
